...
const username = ref('')
const email = ref('')
const token = ref(sessionStorage.getItem('slackchat-token') || '')
const isJoined = ref(false)
const currentChannel = ref(sessionStorage.getItem('slackchat-channel') || 'general')
...
watch(token, (val) => {
  if (val) {
    sessionStorage.setItem('slackchat-token', val)
  } else {
    sessionStorage.removeItem('slackchat-token')
  }
})

// 当前频道持久化
watch(currentChannel, (val) => {
  sessionStorage.setItem('slackchat-channel', val)
})

// ── 启动时尝试恢复会话 ──
const initAuth = async () => {
  const saved = sessionStorage.getItem('slackchat-token')
  if (!saved) {
